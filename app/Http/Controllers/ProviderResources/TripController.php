...
class TripController extends Controller
{
    ...
    public function pickedup_trips(Request $request, $id)
    {
        ...
        $rating = 5;
        $user_img = url('/') . '/uploads/user/profile/user.png';
        $avatar = '';
        $user = [];
        $user_mobile = '';
        $UserRequest = UserRequest::findOrFail($id);
        ...
        if ($UserRequest) {
            ...
            if ($UserRequest->user_mobile) {
                $user_mobile = $UserRequest->user_mobile;
            }
            if ($UserRequest->user_id != 0) {
                ...
                if ($user->picture) {
                    $user_img = url('/') . '/uploads/user/profile/' . $UserRequest->picture;
                }
                if ($user->avatar) {
                    $avatar = url('/') . '/uploads/provider/profile/' . Auth::user()->avatar;
                }
            }
    ...
    public function dropped_trips(Request $request, $id)
    {
        ...
        if ($UserRequest) {
            ...
            if ($UserRequests1 != NULL) {
                ...
                if ($payment) {
                    $UserRequests1->payment->corporate_id = $UserRequests1->corporate_id;
                }
            ...
            $user = ['user_name' => $user_name, 'rating' => $rating, 'user_img' => $user_img, 'avatar' => $avatar, 'user_notes' => $user_notes, 'user_mobile' => $user_mobile];
            if ($UserRequest->payment_update) {
                $payment_update = $UserRequest->payment_update;
            }
    ...
    public function end_trips(Request $request, $id)
    {
        ...
        if ($UserRequest->user_id != 0) {
            ...
            if ($user->picture) {
                $user_img = url('/') . '/uploads/user/profile/' . $user->picture;
            }
            if ($user->avatar) {
                $avatar = url('/') . '/uploads/provider/profile/' . Auth::user()->avatar;
            }
    ...
    public function cashout_request(Request $request)
    {
        ...
        try {
            ...
            if ($request->amount > $walletBalance) {
                return response()->json([
                    'message' => 'You don\'t have sufficient funds to withdrawal',
                    'success' => 0
                ], 422);
            }
            ...
            if ($cashout) {
                ...
                ProviderWallet::create([
                    'provider_id' => Auth::user()->id,
                    'amount' => $provider_cashout->amount,
                    'mode' => 'Withdrawal requested',
                    'status' => 'Debited',
                    'cashout_id' => $provider_cashout->id,
