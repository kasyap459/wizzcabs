...
function AutocompleteDirectionsHandler(map) {
    ...
    this.stop1PlaceId = null;
    this.stop2PlaceId = null;
    this.travelMode = 'DRIVING';
    var originInput = document.getElementById('origin-input');
    var destinationInput = document.getElementById('destination-input');
    var stop1Address = document.getElementById('stop1_address');
    var stop2Address = document.getElementById('stop2_address');
    ...
    var stop1Latitude = document.getElementById('stop1_latitude');
    var stop1Longitude = document.getElementById('stop1_longitude');
    var stop2Latitude = document.getElementById('stop2_latitude');
    var stop2Longitude = document.getElementById('stop2_longitude');
    ...
    document.getElementById("origin-input").onkeyup = function() {
	var a = document.getElementById("origin-input").value.length;
	if (a > 3 ) {
    ...
    var destinationAutocomplete = new google.maps.places.Autocomplete(
            destinationInput);
    var stop1Autocomplete = new google.maps.places.Autocomplete(
            stop1Address);
    var stop2Autocomplete = new google.maps.places.Autocomplete(
            stop2Address);
    ...
    stop1Autocomplete.addListener('place_changed', function(event) {
        var place = stop1Autocomplete.getPlace();
        ...
        if (place.hasOwnProperty('place_id')) {
            ...
            stop1Latitude.value = place.geometry.location.lat();
            stop1Longitude.value = place.geometry.location.lng();
        } else {
            service.textSearch({
                    query: place.name
            }, function(results, status) {
                if (status == google.maps.places.PlacesServiceStatus.OK) {
                    stop1Latitude.value = results[0].geometry.location.lat();
                    stop1Longitude.value = results[0].geometry.location.lng();
                }
            });
        }
    });
    
    stop2Autocomplete.addListener('place_changed', function(event) {
        var place = stop2Autocomplete.getPlace();
        ...
        if (place.hasOwnProperty('place_id')) {
            ...
            stop2Latitude.value = place.geometry.location.lat();
            stop2Longitude.value = place.geometry.location.lng();
        } else {
            service.textSearch({
                    query: place.name
            }, function(results, status) {
                if (status == google.maps.places.PlacesServiceStatus.OK) {
                    stop2Latitude.value = results[0].geometry.location.lat();
                    stop2Longitude.value = results[0].geometry.location.lng();
                }
            });
        }
    });
