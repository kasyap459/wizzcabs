@extends('partner.dispatch.layout.base')
...
<div class="site-sidebar-second custom-scroll custom-scroll-dark">
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="tab-1" role="tabpanel">
            ...
            @include('partner.dispatch.create')
        </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">@lang('partner.triplist.trip_details')</h4>
      </div>
      <div class="modal-body">
        <div id="map1"></div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">@lang('partner.triplist.close')</button>
      </div>
    </div>
</div>
</div>
...
<script>
    $( "#mobile" ).autocomplete({
          source : '/partner/dispatch/users-phone',
          minlenght:3,
          autoFill:true,
    ...
    $( "#email" ).autocomplete({
          source : '/partner/dispatch/users-email',
          minlenght:3,
          autoFill:true,
          select:function(e,ui){
    ...
    $( "#corporate_name" ).autocomplete({
          source : '/partner/dispatch/corporate-user',
          minlenght:3,
          autoFill:true,
          select:function(e,ui){
    ...
    $("#corporate_trip").submit(function(event){
      $('.alert').hide();
        ...
        $.ajax({
            url: '/partner/dispatch/corporate',
            dataType: 'json',
            headers: {'X-CSRF-TOKEN': window.Laravel.csrfToken },
            type: 'POST',
    ...
    function viewer(id){
        $("#loader").show();
        $("#viewer").empty();
        $.get('/partner/dispatch/viewtrip/' +id, function( data ) {
                $("#viewer").html(data);
                $("#loader").hide();
           });
    }
    $(document).on('click','#assign', function() {
        ...
        $.get('/partner/dispatch/assign/' +id+'/'+provider_id, function( data ) {
                activeTab('tab-2');
                ride_updates();
           });
    });
      ...
      var choose_service = $('#choose_service').val();
        ...
        $.ajax
            ({
              ...
              headers: {'X-CSRF-TOKEN': window.Laravel.csrfToken },
              type: "GET",
              url: "/partner/dispatch/driver-list",
              success: function(data)
              {
    ...
    function ride_updates(){
        $.ajax
            ({
              cache: false,
              type: "GET",
              url: "/partner/dispatch/ride-list",
              success: function(data)
              {
                 $('.ride_list').html(data);
              }
...
<!-- <script>
 ...
 --><script>
    ...
    function ajaxMapData(){
        ...
        $.ajax({
            url: "{{ route('partner.dispatch.map') }}",
            dataType: "JSON",
            data: dataString,
            headers: {'X-CSRF-TOKEN': window.Laravel.csrfToken },
            type: "GET",
