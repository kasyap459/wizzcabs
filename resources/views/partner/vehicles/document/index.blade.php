    ...
    <div class="container-fluid">
        <div class="row bg-title">
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                <h4 class="page-title">{{ $vehicle->vehicle_name }} Documents</h4><a href="{{ route('partner.vehicle.index') }}" class="btn btn-outline-warning btn-rounded w-min-sm m-l-0-75 waves-effect waves-light">List Vehicle</a>
            </div>
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                        <form action="{{ route('partner.vehicle.document.upload', [$vehicle->id, $document->id]) }}" method="POST" enctype="multipart/form-data" id="form-upload">
                          {{ csrf_field() }}
                        ...
                        <td>
                            ...
                            @foreach($vehicledocuments as $Index =>$vehicledocument)
                                @if($document->id == $vehicledocument->document_id) 
                                   <a href="{{ route('partner.vehicle.document.edit', [$vehicle->id, $vehicledocument->id]) }}" class="btn btn-sm btn-info btn-rounded b-a-0 waves-effect waves-light">@lang('admin.member.view')</a>
                                   <form action="{{ route('partner.vehicle.document.destroy', [$vehicle->id, $vehicledocument->document_id]) }}" method="POST" id="form-delete">
                                       {{ csrf_field() }}
                                       {{ method_field('DELETE') }}
