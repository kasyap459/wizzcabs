...
<div class="content-area py-1">
    <div class="container-fluid">
        ...
        <div class="box box-block bg-white">
            <h5 class="mb-1">@lang('admin.member.driver_name'): {{ $Document->vehicle->vehicle_name }}</h5>
            <embed src="{{ asset('storage/'.$Document->url) }}" width="70%" height="70%" />

            <div class="row">
                <div class="col-xs-2">
                    <form action="{{ route('partner.vehicle.document.update', [$Document->vehicle->id, $Document->document_id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                ...
                <div class="col-xs-3">
                    <form action="{{ route('partner.vehicle.document.destroy', [$Document->vehicle->id, $Document->document_id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
