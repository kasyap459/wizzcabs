...
@section('content')
    ...
    <div class="content-area py-1">
		...
		<div class="text-center">
				<h3>Total Balance</h3>
                    <img src="{{asset('asset/img/wallet.png')}}"  width="150">
                   
				<h3 class="mt-0">{{ Setting::get('currency') }}<span class="revenue"> {{$wallet}}</span></h3>
            <div class="text-left row" style="margin-left:42%;">
            <form class="" action="{{route('admin.userwallet.debited')}}" method="POST" enctype="multipart/form-data" role="form">
            {{csrf_field()}}
			...
			<form class="" action="{{route('admin.userwallet.store')}}" method="POST" enctype="multipart/form-data" role="form">
			{{csrf_field()}}
			<div class="text-left row" style="margin-left:30%;">
				<div class="col-md-12 row">
					<label style="position: relative; left:15px;top:9px;margin-right:10px;">{{ Setting::get('currency') }} </label><input type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" id="amount" name="amount" style="border-left:0px;border-right:0px;border-top:0px;width:80px" required>
