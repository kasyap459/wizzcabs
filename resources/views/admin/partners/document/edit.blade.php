...
@section('title', 'Sub-company Documents ')
...
<div class="content-area py-1">
    <div class="container-fluid">
        ...
        <div class="box box-block bg-white">
            <h5 class="mb-1">Sub-company name: {{ $Document->partner->name }}</h5>
            <embed src="{{ asset('storage/'.$Document->url) }}" width="70%" height="70%" />

            <div class="row">
                <div class="col-xs-2">
                    <form action="{{ route('admin.partner.document.update', [$Document->partner->id, $Document->document_id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                ...
                <div class="col-xs-3">
                    <form action="{{ route('admin.partner.document.destroy', [$Document->partner->id, $Document->document_id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
