...
<div class="content-area py-1">
    <div class="container-fluid">
        ...
        <div class="box box-block bg-white">
            <h5 class="mb-1">Corporate name: {{ $Document->corporate->name }}</h5>
            <embed src="{{ asset('storage/'.$Document->url) }}" width="70%" height="70%" />

            <div class="row">
                <div class="col-xs-2">
                    <form action="{{ route('admin.corporate.document.update', [$Document->corporate->id, $Document->document_id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                ...
                <div class="col-xs-3">
                    <form action="{{ route('admin.corporate.document.destroy', [$Document->corporate->id, $Document->document_id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
