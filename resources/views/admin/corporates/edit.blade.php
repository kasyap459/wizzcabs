    	...
    	<div class="row bg-title">
            ...
            <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                <ol class="breadcrumb">
                    ...
                    <li class="active">Update corporate</li>
                </ol>
            </div>
        </div>

    	<div class="box box-block bg-white">
			<h5>Update corporate</h5>
            ...
            <form class="form-horizontal" action="{{route('admin.corporate.update', $corporate->id )}}" method="POST" enctype="multipart/form-data" role="form">
				...
				<div class="form-group row">
					...
					<div class="col-xs-8">
						<input class="form-control" type="text" value="{{ $corporate->legal_name }}" name="legal_name" required id="legal_name" placeholder="Legal Name">
					</div>
				</div>
				<div class="form-group row">
					<label for="display_name" class="col-xs-12 col-form-label">Display Name</label>
					<div class="col-xs-8">
						<input class="form-control" type="text" value="{{ $corporate->display_name }}" name="display_name" id="display_name" placeholder="Display Name">
					</div>
				</div>
				<div class="form-group row">
					<label for="email" class="col-xs-12 col-form-label">@lang('admin.member.email')</label>
					<div class="col-xs-8">
						<input class="form-control" type="email" required name="email" value="{{ $corporate->email }}" id="email" placeholder="@lang('admin.member.email')">
					</div>
				</div>
				<div class="form-group row">
					<label for="secondary_email" class="col-xs-12 col-form-label">Secondary email</label>
					<div class="col-xs-8">
						<input class="form-control" type="email" name="secondary_email" value="{{ $corporate->secondary_email }}" id="secondary_email" placeholder="Secondary email">
					</div>
				</div>
				<div class="form-group row">
				...
				<div class="form-group row">
					...
					@if(isset($corporate->picture))
                    	<img style="height: 90px; margin-bottom: 15px; border-radius:2em;" src="{{ asset('storage/'.$corporate->picture) }}" >
                    @endif
					...
					<div class="col-xs-8">
						<select name="country_id" id="country_id" class="form-control">
							...
							@foreach($countries as $country)
								<option value="{{ $country->countryid }}" @if($country->countryid == $corporate->country_id) selected @endif>{{ $country->name }}</option>
							@endforeach
						</select>
					</div>
				</div>
				...
				<div class="form-group row">
                    ...
                    <div class="col-xs-8">
                        <input class="form-control" type="text" onfocus="initMap()" value="{{ $corporate->address }}" name="address" id="address" placeholder="Address" required>
				...
				<div class="form-group row">
					...
					<div class="col-xs-8">
						<input class="form-control" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="{{ $corporate->mobile }}" name="mobile" required id="mobile" placeholder="@lang('admin.member.mobile')">
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-xs-8">
						<select class="form-control" id="notify_customer" name="notify_customer">
							<option value="1" @if($corporate->notify_customer ==1) selected @endif>Enabled</option>
							<option value="0" @if($corporate->notify_customer ==0) selected @endif>Disabled</option>
						</select>
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-xs-8">
						<button type="submit" class="btn btn-success"> <i class="fa fa-check"></i> Update corporate</button>
