    ...
    <div class="container-fluid">
    	...
    	<div class="row bg-title">
            <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
                <h4 class="page-title">SMS Notification to Passenger</h4><a href="{{ route('admin.sms.driver.index') }}" class="btn btn-outline-warning btn-rounded w-min-sm m-l-0-75 waves-effect waves-light">Driver Notification</a>
            </div>
        </div>

    	<div class="box box-block bg-white">
            <form class="form-horizontal" action="{{route('admin.sms.store')}}" method="POST" enctype="multipart/form-data" role="form">
            	{{csrf_field()}}
				<div class="form-group row">
					<label for="select_drivers" class="col-xs-12 col-form-label">Select Passengers</label>
            		<div class="col-xs-10">
				...
				<div class="form-group row">
					...
					<select id='public-methods' name="users[]" multiple='multiple'>
					  ...
					  <option value='{{$user->id}}'>{{$user->first_name}} ({{$user->dial_code}} {{$user->mobile}})</option>
					  @endforeach
					</select>
				</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						...
						<a href="{{route('admin.sms.index')}}" class="btn btn-inverse waves-effect waves-light">@lang('admin.member.cancel')</a>
					</div>
				</div>
			</form>
		</div>
