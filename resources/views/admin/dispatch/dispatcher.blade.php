...
<div class="driver-options custom-scroll custom-scroll-dark opened" style="overflow: hidden !important;">
<div class="" style="background: white;z-index: 1000 !important;opacity: 10;">
   </div>
...
<!-- <script>
  ...
  var firebaseConfig = {
    apiKey: "AIzaSyAKUZ05oh3h9Ni1dV2J_C41_ni-jEvLdEo",
    authDomain: "unicotaxiplus.firebaseapp.com",
    databaseURL: "https://unicotaxiplus.firebaseio.com",
    projectId: "unicotaxiplus",
    storageBucket: "unicotaxiplus.appspot.com",
    messagingSenderId: "6491424797",
    appId: "1:6491424797:web:bccef67e301de42d6adef4",
    measurementId: "G-BLYYCMP961"
  };
  // Initialize Firebase
...
<script type="text/javascript">
   ...
   /* window.vx = {!! json_encode([
    ...
    onSelect: function() {
                ...
                var current_time=<?php echo json_encode(\Carbon\Carbon::now()->addMinutes(30)->format('d-m-Y g:i A'))?>;
        
                if($('#schedule_time').val() >= current_time){
                    $('.schedule_time_block').hide();
                }else{
                    var setting_time="30";
                    $('.set_time').html('Only trip accept after '+setting_time+' minites. ');
...
<script>
    ...
    $( "#corporate_name" ).autocomplete({
          source : '/admin/dispatch/corporate-user',
          minlength:3,
          autoFill:true,
          select:function(e,ui){
	...
	if($("#provider_auto_assign").prop("checked") == false){
        ...
        $.ajax({
			...
			$("#notedata").find("embed").remove();
			$('#notedata').append('<embed src="{{asset('asset/img/notification.mp3')}}" loop="true" autostart="true" width="2" height="0">');
    ...
    function driver_updates(){
      ...
      var data_driverString = "status="+status+"&choose_service="+choose_service+"&choose_corporate="+choose_corporate;
...
<!-- <script>
 ...
 --><script>
    ...
    function worldMapInitialize() {
        ...
        latitude = parseFloat("{{ Auth::guard('admin')->user()->admin_lat }}");
        longitude = parseFloat("{{ Auth::guard('admin')->user()->admin_long }}");
        zooming = parseInt("{{ Auth::guard('admin')->user()->admin_zoom }}");
      ...
      // interval = setInterval(ajaxMapData, 5000);
        ...
        // worldinterval = setInterval(worldMapInitialize, 600000);
    }
    ...
    function ajaxMapData(){
        ...
        if(filterarray.length >0){
            ...
            filters = ["active", "riding", "offline","person"];
        }
        var dataString = "filters="+filters;
        console.log(dataString);
        $.ajax({
            url: "{{ route('admin.dispatch.map') }}",
            dataType: "JSON",
            data: dataString,
            headers: {'X-CSRF-TOKEN': window.Laravel.csrfToken },
            type: "GET",
    ...
    //     autocomplete_stop1.addListener('place_changed', function(event) {
    ...
    //                     return;
    //             }
    //             stop1_latitude.value = place.geometry.location.lat();
    //             stop1_longitude.value = place.geometry.location.lng();
    //             stop1 = new google.maps.LatLng(place.geometry.location.lat(), place.geometry.location.lng());
    //             // updateRoute();
    //         }
    //     });
    ...
    //         // markerStop1.setPosition(result.routes[0].legs[0].end_location);
...
<script>
    $(document).on('click','#drivers', function(){
        var id = $(this).attr("data-id");
        var driver_status=$(this).attr("data-status");
        if(driver_status=="offline"){

//         var db = firebase.database();
//         var ref = db.ref('current_ride').orderByChild('driverId').equalTo(id);
...
//            } else {
//               console.log('There is no user who has driver like '+ id)
//            }

//         $("#livemodalmap").modal("toggle");
//         var position = [28.4594965, 77.0266383];
